...
brain_model_classify = load_model('Models/effnet.h5', compile=False)
lung_model = load_model('Models/lung_final.h5', compile=False)
brain_model = load_model('Models/brain_final.h5', compile=False)
retina_model = load_model('Models/retina_final.h5', compile=False)
liver_model = load_model('Models/liver.h5', compile=False)
polyp_model = load_model('Models/polyp_model.h5', compile=False)


def predict(img, cimg, clas, himg):
    y = 1
    if(clas == "LUNG"):
        ...
        class_result = brain_model_classify.predict(cimg)
        class_result = np.argmax(class_result, axis=1)
        print(class_result)
        prediction_image = prediction.reshape((256, 256))
...
def predict_nii(img, place):
    ...
    img_size = 512
    data = zoom(data, [img_size / data.shape[0],
                img_size / data.shape[1], 1.0], order=1)
    ...
    for i in tqdm(range(data.shape[-1]), "pred: "):
        pred[..., i] = liver_model.predict(np.expand_dims(np.expand_dims(
            np.expand_dims(data[..., i], axis=0), axis=-1), axis=0))[0, ..., 1]
    del data
    ...
    pred = zoom(pred, [curr_shape[0] / img_size,
                curr_shape[1] / img_size, 1.0], order=1)
...
def predict_tif(img):
    prediction = polyp_model.predict(np.expand_dims(img, axis=0))[0] > 0.5
    mask = np.squeeze(prediction)
    mask = [mask, mask, mask]
    mask = np.transpose(mask, (1, 2, 0))
    prediction_image = mask* 255.0
